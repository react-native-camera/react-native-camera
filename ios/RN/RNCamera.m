...
- (void)takePicture:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    ...
    if (@available(iOS 10.0, *)) {
        self.aisaacCaptureOptions = [options mutableCopy];
        self.aisaacCaptureResolve = resolve;
        self.aisaacCaptureReject = reject;
        AVCapturePhotoSettings* settings = [[AVCapturePhotoSettings alloc] init];
        [self.aisaacCameraOutput capturePhotoWithSettings:settings delegate:self];
        return;
    }
    // ↑ 追加
...
- (void)startSession
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        // ↓ 追加
        if (@available(iOS 10.0, *)) {
            self.aisaacCameraOutput = [[AVCapturePhotoOutput alloc]init];
            if ([self.session canAddOutput:self.aisaacCameraOutput]) {
                [self.session addOutput:self.aisaacCameraOutput];
            }
        } else {
            AVCaptureStillImageOutput *stillImageOutput = [[AVCaptureStillImageOutput alloc] init];
            if ([self.session canAddOutput:stillImageOutput]) {
...
API_AVAILABLE(ios(10.0)){
    NSMutableDictionary *options = [self.aisaacCaptureOptions mutableCopy];
    ...
    if (photoSampleBuffer && !error) {
        ...
        // get image metadata so we can re-add it later
        // make it mutable since we need to adjust quality/compression
        CFDictionaryRef metaDict = CMCopyDictionaryOfAttachments(NULL, photoSampleBuffer, kCMAttachmentMode_ShouldPropagate);
        ...
        if (CGImageDestinationFinalize(destination)){
            ...
            if (useFastMode) {
                ...
                self.aisaacCaptureResolve(response);
            }
        }
        else{
            self.aisaacCaptureReject(@"E_IMAGE_CAPTURE_FAILED", @"Image could not be saved", error);
        }
        ...
        self.aisaacCaptureReject(@"E_IMAGE_CAPTURE_FAILED", @"Image could not be captured", error);
    }
}
